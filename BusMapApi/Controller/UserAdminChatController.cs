...
namespace BusMapApi.Controllers
{
    ...
    public class UserAdminChatController : ControllerBase
    {
        ...
        public UserAdminChatController(ApplicationDBContext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<UserAdminChat>> CreateChat([FromBody] UserAdminChat chat)
        {
            try
            {
                ...
                // ❌ XÓA đoạn kiểm tra chat đã tồn tại
        ...
        public async Task<ActionResult<List<UserAdminChat>>> GetChatsByAdminId(int adminId)
        {
            try
            {
                ...
                var adminAccount = await _context.Admin.FirstOrDefaultAsync(a => a.Id == adminId);
        ...
        public async Task<ActionResult<UserAdminMessage>> SendMessage([FromBody] UserAdminMessage message)
        {
            try
            {
                ...
                {
                    senderAccount = await _context.Admin.FirstOrDefaultAsync(a => a.Id == chat.AdminId);
                }
                else
                {
                    return BadRequest("SenderRole không hợp lệ. Chỉ chấp nhận 'User' hoặc 'Admin'.");
                }
